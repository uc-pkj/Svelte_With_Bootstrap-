<script>
	...
	export let chg_question_review = clicked_question;
...
{#if show}
	<div class="text-dark">
		<nav
			id="navbar"
			class="w-25 position-fixed top-0 start-0 bg-white border-right border border-2 border-secondary h-100 p-2"
			...
			{#each $savedData as item, i}
				<small>
					...
					<a
						...
						on:click={() => handleClick((clicked_question = i))}
						>
						<strong>Q{i + 1}. </strong>
